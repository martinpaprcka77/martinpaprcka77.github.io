...
if (Get-Command starship -ErrorAction SilentlyContinue) {
    ...
    # Vendor-provided init script; evaluate as a scriptblock instead of Invoke-Expression.
...
elseif (Get-Command oh-my-posh -ErrorAction SilentlyContinue) {
    ...
    if (Test-Path $poshTheme) {
        & ([scriptblock]::Create((oh-my-posh init pwsh --config $poshTheme | Out-String)))
    }
    else {
        & ([scriptblock]::Create((oh-my-posh init pwsh | Out-String)))
    }
}
#endregion
