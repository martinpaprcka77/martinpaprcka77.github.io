...
.DESCRIPTION
    Shows status of: Dotfiles profiles, VS Code configs,
...
function Watch-SystemMetrics {
    ...
    while ($SampleCount -eq 0 -or $count -lt $SampleCount) {
        ...
        try {
            $memObj = Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction SilentlyContinue
            if ($memObj) {
                ...
                Write-Host "  (CIM not available)" -ForegroundColor Gray
            }
...
function Show-Status {
    ...
    # Richer environment view from the shared toolkit, when it is loaded
    # (Import-Module <root>/toolkit/Toolkit/Toolkit.psd1). Single source for
    # PATH/admin/profile analysis — no duplicated logic here.
    if (Get-Command Get-ShellInfo -ErrorAction SilentlyContinue) {
        $shell = Get-ShellInfo
        Dot 'Elevation'          $(if ($shell.User.IsAdmin) { '✅' } else { '⚠️' }) $(if ($shell.User.IsAdmin) { 'elevated' } else { 'standard user' })
        Dot 'PATH duplicates'     $(if ($shell.Path.Duplicates.Count -eq 0) { '✅' } else { '⚠️' }) "$($shell.Path.Duplicates.Count) duplicate(s)"
        Dot 'PATH missing dirs'   $(if ($shell.Path.Missing.Count -eq 0) { '✅' } else { '⚠️' }) "$($shell.Path.Missing.Count) missing"
        Dot 'Dot-sourced profiles' '✅' "$($shell.DotSources.Count) file(s)"
    }
