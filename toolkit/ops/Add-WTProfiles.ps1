...
$baseProfiles = @(
    ...
    @{
        name         = 'Menu'
        commandline  = "pwsh.exe -NoExit -Command `"& '$toolsRoot\Toolkit\Public\Menu\menu-main.ps1'`""
...
# ── Color schemes — read from config/wt-schemes.json, the single source
...
$schemesFile = Join-Path $toolsRoot 'config\wt-schemes.json'
...
if ($PSCmdlet.ShouldProcess($fragmentPath, 'Create JSON fragment')) {
    ...
    # Also save copy to ops/ for reference
