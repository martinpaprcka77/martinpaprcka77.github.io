...
.PARAMETER Items
    ...
    - With desc/detector: key = @{ Action = { scriptblock }; Desc = "What it does";
      Detector = { @{ Icon = '✅'|'⚠️'|'❌'; Text = '...' } }; Exit = $true }
    Detector is optional. 'Exit = $true' closes the menu after the action —
    required for items like "Exit" in an -Inline menu, where a bare
    '{ return }' action is a no-op (return inside '& $action' does not
    propagate to this function).
    Detector is re-evaluated once per render frame (every
...
function Show-Menu {
    ...
    foreach ($k in $Items.Keys) {
        $v = $Items[$k]
        if ($v -is [scriptblock]) {
            $normalized[$k] = @{ Action = $v; Desc = ''; Exit = $false }
    ...
    # Keys already follow the [ordered] insertion order (the intended display
    # order and the numbers in the labels). Sort-Object sorted them as
    # strings, which put '10.' before '2.' once a menu grew past 9 items.
    ...
    } catch { Write-Debug "menu color config unavailable: $_" }
