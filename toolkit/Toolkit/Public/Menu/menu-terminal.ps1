...
function Show-TerminalMenu {
    ...
    # ops/Add-WTProfiles.ps1 lives inside THIS repo — $env:DOTFILES_TOOLS is only ever set by
    ...
    $items = [ordered]@{
        ...
        '2. 🔄 Generate/Update'  = @{ Action = {
            $s = Join-Path $toolsRoot 'ops\Add-WTProfiles.ps1'
            if (Test-Path $s) { & $s } else { Write-Err "Not found" }
            Read-Host "`nStiskni Enter..."
        }; Desc = 'Create/update JSON fragment with 3 profiles + schemes' }
        ...
        '4. ♻️  Restore Fragment' = @{ Action = {
            ...
            $backups = Get-ChildItem $dir -Filter 'dotfiles.json.backup.*' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending
